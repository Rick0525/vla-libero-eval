...
- [x] **看片两件收官（8/06 晚，Rick 全部裁决；8/06 拍板④销账）**：①**oft_rest 31 段**（t1–t8 零散失败 18 布局）：**零指代混淆 → OFT 两臂×两跑全部失败 100% 看片、全谱无一例混淆，「对称目标混淆=π0.5(3.3B) 特有」终审落定**。机制构成：抓取 12 / 运送掉落 6（t7 瞬时松爪家族延伸 t2/t3/t6/t8）/ 放置 0；**「双碗同抓」子模式现身 7.5B 健康物理**（t5 五布局明确记录；W2 在 0.45B 命名——容量压低发生率不消灭机制，继「偏心抓→落点边缘」后第二个跨容量家族）；t5 四组翻转对两组再现「同一行为内部」边界（两跑都双碗同抓，成功跑靠 ramekin 中途意外脱落得分）；t5 e22/e33 跨臂反向复核毕（两侧皆接触几何、无更深信号，定性=两臂接触边缘安全边距不同，收案）。②**SmolVLA t9 全录像补跑 @3.2.7**（41/50 与主表逐分持平）9 失败全看片：**零混淆（8 H2 + 1 斜爪抓→放置 H3）→「0.45B 不犯灶台/柜顶混淆」t7/t9 双侧验证毕**。leaderboard_zh 五处更新（日期戳/头条 bullet 复核收案/归因节两 bullet/局限两条销账）；裁决档 `failure_videos/oft_rest/`、`smolvla_t9_mj327/`。项目 A 剩余=issue 发射+英文版同步+W7 打包
