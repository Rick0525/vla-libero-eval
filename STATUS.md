# STATUS（开发期工作日志，保留作过程记录）

> 本文件是项目全程的逐日实验记账与拍板记录，按内部口径书写（「开牌」「销账」等行话不另注释）；对外结论与最新数字以 README 与 results/ 为准。更新规则：进度一变就更新本文件。

## 当前状态：W7（2026-08-26）

**头条：repo 转公开（8/26 Rick 拍板即日执行）**：W7 打包收官——README 中文主切换落地（8/25 拍板补 commit）+ 双语三处同步；失败机制表措辞拍定（README 定性层「已判读失败全属此族／发生率低／发生率低，主要失败来源」+ leaderboard 证据层「发生率低（t5 45/50）」，「残留同家族」歧义表述废除）；微调报告钉现状框（历史记录定位，最新数字指向 leaderboard 87.4@n=500@3.2.7）；STATUS 定位拍板=留着+本头注。全历史 75 提交红线扫描（服务器 IP/端口/用户名）零命中。

（历史头条：leaderboard v2 定稿 @3.2.7 同秤 87.4/94.4/97.4、init3 毒布局定谳为版本伪影（8/06）；v1 榜 93.8/83.8 @3.8.1（7/31）；SmolVLA 30k/82.0 @n=100（7/20）。）
...
- [x] 顺手上游素材：HuggingFaceVLA/libero meta per-episode 指针 1690/1693 全错——**8/9 以补充评论落地 [discussion #10](https://huggingface.co/datasets/HuggingFaceVLA/libero/discussions/10)**（他人 8/04 已报案，我方补全量统计 + split 首崩 + 手工 parquet workaround）
...
- [x] **收官范围拍板（8/06 下午，Rick）**：①Exp T 尾巴四条销账（见 W2 三层归因条目尾注）；②**套件范围选 (a) 不扩**——Object/Goal/Long 与 GR00T 不纳入，简历口径改「LIBERO-Spatial 深度评测：leaderboard + 版本敏感度发现 + 三层归因」（深挖故事优于裸广度；SmolVLA 行扩套件需逐套重训，结构上不可行）；③EGL 非确定性两案**不发上游 issue（8/06 晚 Rick 拍板）**，素材转 W7 博客；④剩余看片两件排上：oft_rest 31 段（`results/failure_videos/oft_rest/`，已在库）+ SmolVLA task9（失败集从未录像，8/06 已发射 t9 全录像重跑 @3.2.7 GPU2，`run_smolvla_t9_fullvideo_mj327_20260806.sh`）
- [x] **看片两件收官（8/06 傍晚，Rick 全部裁决；同日订正 e22——本条为订正后版本）**：①**oft_rest 31 段**（t1–t8 零散失败 18 布局）：OFT 两臂×两跑全部失败至此 **100% 看片**；17/18 布局无混淆，**1 例定谳指代混淆（8/06 订正：Rick 复看合训 t5 e22 发现拿错碗——两跑均直奔 cookie box 干扰碗、目标碗从未被接近、干扰碗也未抓起（戳倒/捅落），Claude 帧核证实 0730 同款+布局锁定；证据图 e22_*.png 入档）**。「OFT 全谱零混淆」不成立，修正口径：**系统性灶台/柜顶对称混淆仍系 π0.5(3.3B) 特有；OFT 特训臂全谱零混淆、合训臂 1/500 单布局孤例、SmolVLA 零——混淆维度并入「容量压低发生率不消灭机制」家族**；e22 跨臂反向据此升级为 grounding 层分岔（合训臂独有质性混淆孤例，n=1 不外推；总分合训税=0 不变）。其余 17 布局机制：抓取 11 / 运送掉落 6（t7 瞬时松爪家族延伸 t2/t3/t6/t8）/ 放置 0；**「双碗同抓」子模式现身 7.5B 健康物理**（t5 五布局明确记录；W2 在 0.45B 命名——容量压低发生率不消灭机制的又一家族）；t5 四组翻转对两组再现「同一行为内部」边界（两跑都双碗同抓，成功跑靠 ramekin 中途意外脱落得分）。②**SmolVLA t9 全录像补跑 @3.2.7**（41/50 与主表逐分持平）9 失败全看片：**零混淆（8 H2 + 1 斜爪抓→放置 H3）→「0.45B 不犯灶台/柜顶混淆」t7/t9 双侧验证毕**。leaderboard_zh 更新（日期戳/头条 bullet/归因节/局限两条销账，含深夜订正四处）；裁决档 `failure_videos/oft_rest/`、`smolvla_t9_mj327/`。项目 A 剩余=issue 发射+英文版同步+W7 打包
- [x] **上游 issue 包五发全部落地（8/9，Rick 逐份过审+网页亲发，Claude 查证/核对）**：主 issue 改靶 [lerobot#4390](https://github.com/huggingface/lerobot/issues/4390)（huggingface/LIBERO 系 fork 且 Issues 关闭）+ [LRL#141 回帖](https://github.com/Lifelong-Robot-Learning/LIBERO/issues/141#issuecomment-5231993900) + lerobot#2114 交叉引用 + [RLinf#1460](https://github.com/RLinf/RLinf/issues/1460) 钉版（发现 RLinf「半知情」：#1302 系 <=3.9.0 上限来源、#1375 维护者已个案劝降 3.3.2 只知视觉通道）+ HFVLA discussion #10 补充评论；探针单文件版公开 [gist](https://gist.github.com/Rick0525/6e6db2d1fe5f4358c980b569b123fde8)；zoom 主证图重制（上下排版，`scripts/make_task5_zoom_figure.py`）。草稿与发帖手册详档：VLA_Lab 主仓 `upstream_prep/libero_mujoco_20260806/`。余下=盯回复接 PR 邀约茬
- [x] **leaderboard_zh 读者版重写 + 审读订正（8/21，Rick 重构、Claude 通读核对，commit `21f4938`）**：v2 内部记账口径（版本戳/头条/网格）改为读者视角八节。Claude 通读挑出 18 条、落地 17 处：**硬错** 幕一 reset 描述（恢复悬空 init → 空转 10 控制步≈0.5 s 落座 → 才取首帧，源码 `libero.py` `num_steps_wait=10`，原稿误写 50 步）、「全部失败均经归因」→ 覆盖率逐模型披露、π0.5 失败机制节 29（3.8.1 期重跑）vs 主表 28 说明、局限首条 171/173 标明为 3.8.1 期两次同栈跑、上游 issue「在途」→ 已提、π0.5 墙钟 65→62 分钟（主表跑 `eval_info.json` `eval_s`=3749 s）、t5 重合 4→4/9；**论证** SmolVLA +18=task5 +26 与其余九任务净 −8 的逐任务构成、π0.5「六任务持平」补 t2/t3/t8、OFT 局限补跨集位置效应并链接案卷 + 复现节「单集复现不等价序列内」、两跑独立重抽依据改为「噪声流起点错开」（`run_pi05_bleed4_video_20260731.sh` 单进程 4 任务 + `lerobot_eval.py:734` set_seed 仅一次 + `sample_noise` 走全局 RNG 核实）、「系统性指代混淆是中间容量特有」降为「只在 π0.5 上观察到，容量/预训练/移植版缺语言分层推理分不开」；**措辞** e22「拿错碗」→「奔向错的碗」、首分叉腕 45/主 5、恢复合训重跑跨卡括注、pi05 行号注版本。未做：英文版整篇同步（仍 v1 结构，仅先同步「稳定步数两栈对称」一条）；上游旁证 LIBERO#141/SimVLA#14 链接留给 README。**→ 8/25 销账：英文版整篇同步完成（Rick 审改定稿，随本 commit 入库）**

### W7（8/25–）：打包与转公开

- [x] **README 语言切换落地（8/25 拍板、8/26 补 commit）+ 双语同步**：中文转正为 README.md 主、README_en.md 辅；英文版补 task5 主证图、标题带上「上游问题发现」
- [x] **失败机制表措辞三轮拍定（8/26，Rick 逐轮驳回/定版）**：「残留同家族/残留失败仍属此族」判为歧义病句（会被读成对 π0.5 全部残留失败的断言，与系统性指代混淆行打架）；分层定版——README 定性层三格「已判读失败全属此族／发生率低／发生率低，主要失败来源」（共享前缀载构成差异、「未见」作表内零标记），leaderboard 证据层「发生率低（t5 45/50）」
- [x] **微调报告钉现状框（8/26）**：历史记录定位；同 checkpoint 8/05 重评 87.4@n=500@3.2.7 指向 leaderboard；「与论文 90 缺口 Open」以 n=500 销账（z=0.73 p=0.47）；W1 双 workaround 补「W2 已摘除」事实
- [x] **转公开（8/26 Rick 拍板）**：STATUS 留着+头注；全历史红线扫描零命中；push 后即转 public
